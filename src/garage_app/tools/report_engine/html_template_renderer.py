...
th { font-weight:700; text-align:left; padding:5px 8px; border-bottom:2px solid #1C1C1E; }
td { padding:5px 8px; border-bottom:1px solid #D1D1D6; }
tr:last-child td { border-bottom:none; }
.num { text-align:right; }
.center { text-align:center; }
.section { margin:12px 0; padding:0 4px; }
.section h2 { font-size:11pt; font-weight:700; text-decoration:underline; padding-bottom:4px; margin-bottom:8px; }
.totaux-table { width:auto; float:right; border:1px solid #1C1C1E; }
.totaux-table td { padding:5px 12px; border-bottom:1px solid #D1D1D6; }
.totaux-table tr:last-child td { border-bottom:none; }
.footer-band { margin-top:16px; font-size:8pt; border-top:1px solid #D1D1D6; padding-top:6px; text-align:center; font-style:italic; }
.badge { display:inline-block; padding:1px 6px; border:1px solid #1C1C1E; font-size:8pt; font-weight:600; }
...
class HtmlTemplateRenderer:
    ...
    def render(self, template: HtmlReportTemplate, context: dict[str, Any]) -> str:
        ...
        if template.css_custom:
            css += "\n" + template.css_custom
        css += "\n.header-band { border-bottom:2px solid #1C1C1E; padding:10px 16px 8px 16px; }"
    ...
    def _header(self, template: HtmlReportTemplate, titre: str, sous_titre: str) -> str:
        ...
        if template.show_societe:
            societe_html = (
                '<div style="font-size:8.5pt; margin-top:2px; font-style:italic;">'
        ...
        if template.show_slogan:
            slogan_html = (
                '<div style="font-size:8pt; margin-top:1px; font-style:italic;">'
                "Solutions de Gestion sur Mesure"
                "</div>"
            )
        sous_titre_html = f'<div style="font-size:9pt; margin-top:2px; font-style:italic;">{sous_titre}</div>' if sous_titre else ""
        return (
    ...
    def _totaux_block(
        ...
        if template.show_ttc:
            rows += (
                f'<tr>'
                f'<td style="padding:7px 12px;font-weight:700;border-bottom:1px solid #D1D1D6;text-decoration:underline;">Total TTC</td>'
                f'<td class="num" style="padding:7px 12px;font-weight:700;font-size:11pt;border-bottom:1px solid #D1D1D6;text-decoration:underline;">{_fmt(ttc)}</td></tr>'
            )
        if template.show_paye:
            rows += (
                f'<tr><td style="padding:5px 12px;">Montant payé</td>'
                f'<td class="num" style="padding:5px 12px;font-weight:700;">{_fmt(paye)}</td></tr>'
            )
        if template.show_reste:
            reste_lbl = "Reste à payer" if float(reste) > 0 else "Soldé ✓"
            reste_style = "font-weight:700; font-style:italic;" if float(reste) > 0 else "font-style:italic;"
            rows += (
                f'<tr><td style="{reste_style}padding:5px 12px;">{reste_lbl}</td>'
                f'<td class="num" style="{reste_style}padding:5px 12px;">{_fmt(reste)}</td></tr>'
            )
